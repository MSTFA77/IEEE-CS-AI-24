...
class StudentDBApp:
    def __init__(self, master):
        ...
        font_size = min(screen_width, screen_height) // 40
        self.custom_font = ("Helvetica", font_size)
        self.db = StudentDB()
        self.button_width = 20
        self.create_widgets()

    def create_widgets(self):
        self.master.config(bg="#440D0F")
        self.label = tk.Label(self.master, text=" Student DataBase Management ", fg="#FFFDFC", bg="#191716", font=self.custom_font)
        self.label.pack()
        button_bg = "#4caf50"  
        button_fg = "white"  
        button_padx = 10
    ...
    def add_student(self):
        ...
        add_window.config(bg="#AF9BB6")

        tk.Label(add_window, text="Name:", bg="#53613A", font=("Helvetica", 12), width=self.button_width, fg="#603A40").grid(row=0, column=0)
        tk.Label(add_window, text="Age:", bg="#53613A", font=("Helvetica", 12), width=self.button_width, fg="#603A40").grid(row=1, column=0)
        tk.Label(add_window, text="Grade:", bg="#53613A", font=("Helvetica", 12), width=self.button_width, fg="#603A40").grid(row=2, column=0)
    ...
    def view_students(self):
        ...
        view_window.config(bg="#3D3A61")
        students = self.db.view_students()
        tk.Label(view_window, text=" ID ", bg="#53613A", font=("Helvetica", 12), fg="#603A40", width=self.button_width).grid(row=0, column=0)
        tk.Label(view_window, text=" Name ", bg="#53613A", font=("Helvetica", 12), fg="#603A40", width=self.button_width).grid(row=0, column=1)
        tk.Label(view_window, text=" Age ", bg="#53613A", font=("Helvetica", 12), fg="#603A40", width=self.button_width).grid(row=0, column=2)
        tk.Label(view_window, text=" Grade ", bg="#53613A", font=("Helvetica", 12), fg="#603A40", width=self.button_width).grid(row=0, column=3)
        for i, student in enumerate(students):
            tk.Label(view_window, text=student.id, bg="#3D3A61", font=("Helvetica", 12), width=self.button_width).grid(row=i+1, column=0)
            tk.Label(view_window, text=student.name, bg="#3D3A61", font=("Helvetica", 12), width=self.button_width).grid(row=i+1, column=1)
            tk.Label(view_window, text=student.age, bg="#3D3A61", font=("Helvetica", 12), width=self.button_width).grid(row=i+1, column=2)
            tk.Label(view_window, text=student.grade, bg="#3D3A61", font=("Helvetica", 12), width=self.button_width).grid(row=i+1, column=3)
